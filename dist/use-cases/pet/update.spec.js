...
var PetsRepositoryInMemory = class {
  ...
  async save(data) {
    const index2 = this.items.findIndex((p) => p.id === data.id);
    if (index2 >= 0) {
      this.items[index2] = { ...data };
    }
...
// src/use-cases/pet/update.spec.ts
...
describe("Update Pet Use Case", () => {
  ...
  it("should to be able update a pet", async () => {
    const { id: org_id } = await orgsRepository.create({
      ...
      longitude: 0,
      phone: "22999999",
      password_hash: await (0, import_bcryptjs.hash)("abc123", 6)
    });
    ...
    globalExpect(pet.name).toEqual("Smile dog edited");
    globalExpect(await petsRepository.fetchAll()).toEqual([
      globalExpect.objectContaining({ name: "Smile dog edited" })
    ]);
  });
